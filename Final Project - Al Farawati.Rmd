...
```{r 13 highest naics code}
# Filter the data for the specified revenue categories
vi_manuf2018plus_HighestCode <- vi_manuf2018plus %>%
  filter(revenue_category %in% c("$500M-$1B", "Over $1B"))

# Count the number of occurrences of each naics_code_descr
vi_manuf2018plus_HighestCodeCount <- vi_manuf2018plus_HighestCode %>% 
  group_by(naics_code_descr) %>%
  summarise(count = n()) %>%
  arrange(desc(count))

# Iterate over the rows of the dataframe and print the naics_code_descr and their counts
apply(vi_manuf2018plus_HighestCodeCount, 1, function(row) {
  cat(paste0(row["naics_code_descr"], " got ", row["count"], " times.\n"))
})
