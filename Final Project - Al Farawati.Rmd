...
library(stringr, quietly = TRUE)   # this package is for 
```
...
```{r 12.2 empl size analysis plot}
if(FALSE){
...
yearly_growth$yr <- as.numeric(yearly_growth$yr)
yearly_growth$naics_code_descr <- as.factor(yearly_growth$naics_code_descr)

ggplot(yearly_growth, aes(x = yr, y = growth_rate, color = naics_code_descr)) +
  geom_line() +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.01)) +
  labs(title = "Yearly Growth Rate by Business Category",
       x = "Year",
       y = "Growth Rate (%)",
